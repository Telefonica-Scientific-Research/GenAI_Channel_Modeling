            ...
            for x_test, c_test in pbar:
                ...
                for s in range(batch_size):
                    ...
                    if ccount % 200 == 0 and flag_data_split == "test":
                        # Add a text to each image indicating the column with the maximum value
                        plt.figure(figsize=(10, 5))
                        plt.subplot(2, 1, 1)
                        plt.imshow(magnitude_ground_truth_normalized, cmap='viridis')
                        # Add a text to the image indicating the column with the maximum value
...
def main(mode):
    ...
    n_classes = 3
    n_feat = 256
    save_dir = f'../../GenAI_Channel_Modeling_Models/logs/FMM_sina_dataset_3.5GHz_LoS_0.0_200_14_05_2026_10_21/' # UPDATE THIS::
    ws_test = [0.0]
    n_sample = 1 
    steps = 10
