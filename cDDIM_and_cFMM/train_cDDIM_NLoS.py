...
def train():
    # (cDDIM) python train_cDDIM_NLoS.py 
    n_epoch = 3000
    batch_size = 100
    n_T = 200
    ...
    val_split_init = 0.7
    val_split_end = 0.9
    save_dir = f'{logs_main_dir}CDDIM_dataset_{flag_frequency}GHz_NLoS+LoS_{train_split_init}_{train_split_end}_nT{n_T}/'
    
    indices_path = save_dir# "/logs/CDDIM_dataset_3.5GHz_NLoS+LoS_0.0_200_nT10/"
    ...
    if not os.path.exists(os.path.join(save_dir, f'models_x_epoch')):
        os.makedirs(os.path.join(save_dir, f'models_x_epoch'))
    ...
    # Print number of samples in dataset_train and write to training_log.txt
    ...
    # Print number of samples in dataset_val and write to training_log.txt
    print(f"Number of samples in validation split: {len(dataset_val)}")
    with open(os.path.join(save_dir, 'training_log.txt'), 'a') as f:
        f.write(f"Number of samples in validation split: {len(dataset_val)} with val_split_init: {val_split_init} and val_split_end: {val_split_end}\n")
